...
export const run = async (contexto) => {
  const { reply, react, sock, from, sender } = contexto;
  ...
  const dbUser = await User.findOne({ jid: objetivo, groupId: from }).select("permisos").lean();
  if (!dbUser || dbUser.permisos < 2) return reply(aviso(`@${numFromJid(objetivo)} no es Moderador en este grupo.`), [objetivo]);

  // Quitar permisos en DB
  await User.findOneAndUpdate({ jid: objetivo, groupId: from }, { $set: { permisos: 0 } });
