...
export const run = async (contexto) => {
  const { sender, from, args, sock, msg, react, communityId } = contexto;
  ...
  const dbUser = await User.findOne({ jid: sender, communityId, permisos: 3 }).select("permisos").lean();
  ...
  await User.findOneAndUpdate({ jid: sender, communityId }, { $set: { permisos: 3, groupId: from } }, { upsert: true });
  await react("👑");
